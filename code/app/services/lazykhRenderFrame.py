...
global USE_BILLBOARDS
global ENABLE_JIGGLING
global ENABLE_FRAME_CACHING
global MOUTH_COOR
global BG_CACHE
global schedules
...
def drawFrame(frameNum, paragraph, emotion, imageNum, pose, phoneNum, poseTimeSinceLast, poseTimeTillNext):
    ...
    scribble = None
    if USE_BILLBOARDS:
        FILENAME = f"example_billboards/{getFilenameOfLine(origScript[imageNum])}.png"
        ...
        if scribble is not None:
            ...
            scribble = scribble.resize(
                (int(round(O_scale*s_W)), int(round(O_scale*s_H))), Image.Resampling.LANCZOS)
    ...
    if USE_BILLBOARDS and scribble is not None:
        ...
        img1.rectangle(
            [(s_X+W_M-4, W_M-4), (s_X+W_W/2-W_M+8, W_H-W_M+8)], fill="#603810")
    ...
    jiggleFactor = 1
    if ENABLE_JIGGLING:
        preJF = getJiggle(poseTimeSinceLast, 0.06, 0.6) - \
            getJiggle(poseTimeTillNext, 0.06, 0.6)
    ...
    if MOUTH_COOR[poseIndex, 3] != 1:
        m_W, m_H = mouth.size
        mouth = mouth.resize((int(abs(m_W*MOUTH_COOR[poseIndex, 2])), int(
            m_H*MOUTH_COOR[poseIndex, 3])), Image.Resampling.LANCZOS)
    if MOUTH_COOR[poseIndex, 4] != 0:
        mouth = mouth.rotate(-MOUTH_COOR[poseIndex, 4],
                             resample=Image.Resampling.BICUBIC)

    m_W, m_H = mouth.size
    body.paste(mouth, (int(MOUTH_COOR[poseIndex, 0]-m_W/2),
               int(MOUTH_COOR[poseIndex, 1]-m_H/2)), mouth)
    ...
    INPUT_FILE_Frames = "exampleFrames"
    if FLIPPED:
        body = body.transpose(Image.Transpose.FLIP_LEFT_RIGHT)
    frame.paste(body, (inx-s_X, iny), body)
    if not os.path.isdir(INPUT_FILE_Frames+"_frames"):
        os.makedirs(INPUT_FILE_Frames+"_frames")
    frame.save(INPUT_FILE_Frames+"_frames/f"+"{:06d}".format(frameNum)+".png")
...
def duplicateFrame(prevFrame, thisFrame):
    ...
    prevFrameFile = INPUT_FILE_Frames+"_frames/f" + \
        "{:06d}".format(prevFrame)+".png"
    thisFrameFile = INPUT_FILE_Frames+"_frames/f" + \
...
async def drawer(transcriptFile, scheduleFile):
    ...
    USE_BILLBOARDS = "F"
    ENABLE_JIGGLING = "F"
    ENABLE_FRAME_CACHING = "T"

    f = scheduleFile
    scheduleLines = f.split("\nSECTION\n")
    f.close()
    ...
    for i in range(len(schedules[4])):
        ...
        if i >= 1 and framestamp <= phonemeTimeline[-1][1]:
            if i >= 2 and phonemeTimeline[-2][1] <= framestamp-2:
                phonemeTimeline[-1][1] = framestamp - \
                    1  # shift previous one back
            else:
