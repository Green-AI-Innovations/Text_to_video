...
def drawFrame(
    ...
    if USE_BILLBOARDS:
        FILENAME = (
            f"{classfied_emotion_file}_billboards/{getFilenameOfLine(origScript[imageNum])}.png"
        )
...
def duplicateFrame(prevFrame, thisFrame):
    prevFrameFile = classfied_emotion_file + "_frames/f" + "{:06d}".format(prevFrame) + ".png"
    thisFrameFile = classfied_emotion_file + "_frames/f" + "{:06d}".format(thisFrame) + ".png"
...
parser.add_argument("--classfied_emotion_file", type=str, help="the script")
parser.add_argument("--scheduler_file", type=str, help="the script")
parser.add_argument(
    "--use_billboards", type=str, help="do you want to use billboards or not"
)
parser.add_argument(
    "--jiggly_transitions",
    type=str,
...
for i in range(len(phonemeTimeline) - 1):
    setPhoneme(i)
# classfied emotion file name
f = open(classfied_emotion_file + ".txt", "r+")
...
f = open("text_to_video/lazykh/mouthCoordinates.csv", "r+")
